...
/**
 * Démarre le backend CYBEL dans Termux si nécessaire, puis attend /api/health.
 */
public final class BackendStarter {

    private static final String TAG = "CybelBackend";
    private static final String TERMUX_BASH = "/data/data/com.termux/files/usr/bin/bash";
    private static final String START_CMD =
            "bash /data/data/com.termux/files/home/cybel/scripts/termux/ensure_cybel_backend.sh";
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                if (isBackendHealthy("127.0.0.1") || isBackendHealthy(getWlanIp())) {
                    Log.i(TAG, "Backend déjà actif");
                    postReady(callback);
                    return;
                }

                Log.i(TAG, "Lancement backend CYBEL via Termux…");
                boolean launched = tryTermuxRunCommand(context) || trySuTermuxBash();
                if (!launched) {
                    postFailed(callback,
                            "Impossible de lancer Termux — vérifiez que Termux et CYBEL sont installés.");
                    return;
                }
                ...
                postFailed(callback,
                        "Le backend CYBEL ne répond pas — ouvrez Termux une fois ou vérifiez ~/cybel-uvicorn.log");
            }
        }, "CybelBackendStart").start();
    }
    ...
    private static boolean trySuTermuxBash() {
        String[] commands = new String[]{
                "su",
                "-c",
                TERMUX_BASH + " -lc '" + START_CMD + " >> ~/cybel-app-start.log 2>&1 &'"
        };
        try {
