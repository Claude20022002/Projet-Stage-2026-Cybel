...
the live map, but that is a maintenance argument, not a reliability one. We met the mirror image
of the same trap immediately afterwards, in our own verification script, which watched a status
topic numbered by the underlying planner rather than by the vendor: the robot navigated correctly
and the script saw nothing (Section~\ref{sec:discussion}-A).
