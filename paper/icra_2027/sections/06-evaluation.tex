...
H2 is rejected. Wildcard subscription across several sessions returned only one telemetry topic,
and no payload we replayed changed the robot's state: the vendor library shows its broker client
wired solely to telemetry, the command path running exclusively over the WebSocket bridge. We
report the rejection because MQTT is a reasonable first guess for anyone approaching a machine
like this, and rediscovering that it leads nowhere is expensive.
...
certifies the first hop and nothing beyond it (Fig.~\ref{fig:h4}). Across \num{17} logged
navigation commands, the outcome followed the reported localization state exactly: all ten
failures happened while unlocalized, all seven successes while navigating. The same split holds
for both goal-addressing schemes: coordinate goals failed five times and succeeded once, named
annotations failed five times and succeeded six. The addressing scheme explains nothing here;
the precondition state explains everything, and every one of the failed commands had still been
acknowledged.
...
And in \num{309} legs to its right, exactly one failed: a goal issued, acknowledged, and twenty
seconds later the robot had not moved. That single failure is the H4 event of
Section~\ref{sec:eval}-B, occurring unprompted in service.

\input{tables/tab-metrics}
\input{figures/fig-tour-timeline}

The interaction layer contributes three negative results, bounding what the platform delivers.
Question matching succeeded in 27 of 48 trials (\SI{56.2}{\percent}), the failures being near
misses rather than outright non-matches, arguing for a wider synonym set over a different
threshold. The wake phrase, built from ordinary words so the recogniser would accept it, is
ordinary enough to trigger by accident: \num{20} of \num{55} logged events opened the microphone
with no utterance following, eighteen of them followed by no speech at all, so the cheap remedy
is silence rather than a prompt. Finally, the interaction loop is two distributions, not one:
\SI{1.3}{\second} median for a local lookup ($n=9$), \SI{22}{\second} when it also drives the
robot ($n=13$), because preparation runs inside the loop; averaging them, as an earlier revision
did, describes neither.
