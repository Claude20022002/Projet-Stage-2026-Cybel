...
\num{24} instrumented sessions rather than the handful that succeeded. Completed tours have a
median duration of \SI{10.8}{\minute}. Legs within a tour are not independent, so the leg-level
interval is the optimistic reading. Two things in that figure are worth saying out loud: the
decisive variable is the stability of the map artifacts rather than anything in our stack, and
the only abort mode that survives to the right of the rule is speech, which is precisely the
capability we had to recover outside the protocol (Section~\ref{sec:platform}-C).
