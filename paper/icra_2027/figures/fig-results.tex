...
%
% Panel widths and gaps are sized to stay inside \textwidth (516pt = 18.13cm
% for this class) with margin to spare: 0.32+0.27+0.27 = 0.86\textwidth plus
% two 0.4cm gaps is about 16.4cm, leaving room for the tick/axis-label
% overhang pgfplots adds outside the nominal width. An earlier version at
% 0.36+0.30+0.30 plus fixed 1.15cm/7.2cm offsets from panel (a) alone summed
% past \textwidth and bled into the margin (reviewer-caught). Panel (c) is
% now anchored off panel (b)'s right edge, not panel (a)'s, so the three
% panels chain left to right instead of two competing offsets from one point.
\begin{figure*}[!t]
\centering
\begin{tikzpicture}
...
% ---------------------------------------------------------------- (a) ----
\begin{axis}[
  name=ax1, width=0.32\textwidth, height=4.3cm,
  ...
  xtick=data,
  xticklabels={Teleop. (10),Legs before (7),Legs after (309),Coord. (10),Annot. (10),Question (48)},
...
\begin{axis}[
  name=ax2,
  at={(ax1.right of south east)}, xshift=0.4cm, anchor=left of south west,
  width=0.27\textwidth, height=4.3cm,
...
% ---------------------------------------------------------------- (c) ----
\begin{axis}[
  at={(ax2.right of south east)}, xshift=0.4cm, anchor=left of south west,
  width=0.27\textwidth, height=4.3cm,
