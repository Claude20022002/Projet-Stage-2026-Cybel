...
\IEEEauthorblockA{Centre de Recherche en Ing\'enierie et Management (CERIM)\\
HESTIM Engineering \& Business School, Casablanca, Morocco}
}
...
application, and passive broker observation. We validate the methodology on a CIOT
TY1251D-03195 reception robot, recovering \num{455} topics and \num{308} services and
...
\caption{The CIOT TY1251D-03195 reception robot running our reconstructed visitor interface on
its own touchscreen. The head is an Android computer; the mobile base underneath is a separate
Linux/\ros{} machine. Neither exposes a documented interface to its owner.}
\label{fig:robot}
\end{figure}
...
The CIOT TY1251D-03195 pairs a Linux/\ros{} chassis~\cite{quigley2009ros} carrying SLAM, a
planner, LiDAR and RGB-D sensing, with an Android~7.1 head driving a \SI{15.6}{inch}
touchscreen, cameras and speakers. The two are independent network stacks, a fact no vendor
document states and no application string reveals.
...
We thank the HESTIM robotics laboratory for robot access and support during field testing.
