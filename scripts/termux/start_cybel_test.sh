...
# Démarre le backend CYBEL test sur Termux (écoute 0.0.0.0:8001, ~/cybel-test).
set -euo pipefail
...
LOG_FILE="$HOME/cybel-test-uvicorn.log"
...
write_kiosk_url() {
  ...
  if mkdir -p "$(dirname "$KIOSK_URL_FILE")" 2>/dev/null && echo "$URL" >"$KIOSK_URL_FILE" 2>/dev/null; then
    echo "URL kiosk test pour l'app Android : $URL"
    return 0
  fi

  if command -v su >/dev/null 2>&1; then
    if su -c "mkdir -p /sdcard/Download && printf '%s\n' '$URL' > /sdcard/Download/cybel_kiosk_test_url.txt" 2>/dev/null; then
      echo "URL kiosk test (via su) : $URL"
      return 0
    fi
  fi
  ...
  echo "URL kiosk test : $URL"
  return 0
}

if curl -sf "http://127.0.0.1:$PORT/api/health" >/dev/null 2>&1; then
  echo "CYBEL test déjà actif — mise à jour URL kiosk uniquement"
  write_kiosk_url
  exit 0
fi

if [ ! -d "$CYBEL_HOME/scripts/termux" ]; then
  echo "ERREUR: cybel-test non déployé dans $CYBEL_HOME"
  exit 1
fi
...
if [ -f "$PID_FILE" ]; then
  ...
  if kill -0 "$OLD_PID" 2>/dev/null; then
    echo "Arrêt de l'instance test précédente (pid $OLD_PID)..."
    kill "$OLD_PID" || true
    sleep 1
  fi
  rm -f "$PID_FILE"
